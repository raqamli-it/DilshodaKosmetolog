...
const UserUpdateForm = ({ id }) => {
  ...
  const fetchPhotoFormat = async () => {
    if (!selectedFile && apiData?.photo) {
      try {
        const response = await fetch(apiData.photo);
  ...
  useEffect(() => {
    if (apiData) {
      ...
      setBoolen({
        ...boolens,
        region_name: apiData.region?.name || "Viloyat",
        type_name: apiData.type_disease?.name || "Turi"
      });
  ...
  const removeAppointmentOld = (index) => {
    ...
    setFormData((prev) => ({
      ...prev,
      appointments: updatedAppointments,
      remove: [...prev.remove, removedItem.id]
    }));
  };
